...
const TO_NAME = prettyCityNameFromSlug("velikiy-novgorod");

export const metadata: Metadata = {
  title: `Такси Санкт‑Петербург — ${TO_NAME} | Межгород 24/7 — ${SITE_NAME}`,
  description:
    `Трансфер Санкт‑Петербург — ${TO_NAME}: комфортные авто, подача по времени, стоимость согласуем заранее. Заявка онлайн 24/7.`,
  alternates: { canonical: `${SITE_URL}/sankt-peterburg/velikiy-novgorod` },
  robots: { index: true, follow: true },
  openGraph: {
    type: "website",
    url: `${SITE_URL}/sankt-peterburg/velikiy-novgorod`,
    title: `Такси Санкт‑Петербург — ${TO_NAME} | Межгород 24/7 — ${SITE_NAME}`,
    description:
      `Трансфер Санкт‑Петербург — ${TO_NAME}: комфортные авто, подача по времени, стоимость согласуем заранее. Заявка онлайн 24/7.`,
    siteName: SITE_NAME,
...
export default function Page() {
  ...
  const breadcrumbsJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: `${TO_NAME}`, item: `${SITE_URL}/sankt-peterburg/velikiy-novgorod` },
    ],
  };
  ...
  return (
    <>
      <Script
        id="ld-route-breadcrumbs-sankt-peterburg-velikiy-novgorod"
        type="application/ld+json"
        strategy="beforeInteractive"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbsJsonLd) }}
      />
      <Script
        id="ld-route-faq-sankt-peterburg-velikiy-novgorod"
        type="application/ld+json"
        strategy="beforeInteractive"
