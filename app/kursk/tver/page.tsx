...
export const metadata: Metadata = {
  ...
  alternates: { canonical: `${SITE_URL}/kursk/tver` },
  robots: { index: true, follow: true },
  openGraph: {
    type: "website",
    url: `${SITE_URL}/kursk/tver`,
    title: `Такси Курск — ${TO_NAME} | Межгород 24/7 — ${SITE_NAME}`,
    description:
      `Трансфер Курск — ${TO_NAME}: комфортные авто, подача по времени, стоимость согласуем заранее. Заявка онлайн 24/7.`,
    siteName: SITE_NAME,
...
export default function Page() {
  ...
  const breadcrumbsJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: `${TO_NAME}`, item: `${SITE_URL}/kursk/tver` },
    ],
  };
  ...
  return (
    <>
      <Script
        id="ld-route-breadcrumbs-kursk-tver"
        type="application/ld+json"
        strategy="beforeInteractive"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbsJsonLd) }}
      />
      <Script
        id="ld-route-faq-kursk-tver"
        type="application/ld+json"
        strategy="beforeInteractive"
