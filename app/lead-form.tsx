...
function IconPlane() {
  return <svg viewBox="0 0 24 24" className="h-[17px] w-[17px]" fill="none" stroke="currentColor" strokeWidth="2"><path d="M2 19l20-7-20-7 5 7-5 7Z" /><path d="M7 12h15" /></svg>;
}
function IconCar() {
  return <svg viewBox="0 0 24 24" className="h-[17px] w-[17px]" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 16l1.4-5A2 2 0 0 1 8.3 9h7.4a2 2 0 0 1 1.9 2L19 16" /><path d="M3 16h18" /><circle cx="7" cy="17" r="1.7" /><circle cx="17" cy="17" r="1.7" /></svg>;
}
function IconCity() {
  return <svg viewBox="0 0 24 24" className="h-[17px] w-[17px]" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 21h18" /><path d="M5 21V9l6-3v15" /><path d="M11 21V4l8 3v14" /></svg>;
}
function IconPin() {
...
function TypeTab({ active, label, icon, onClick }: { active: boolean; label: string; icon: ReactNode; onClick: () => void }) {
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "flex h-12 items-center justify-center gap-1.5 rounded-[18px] px-2 text-[13px] font-extrabold transition sm:h-12 sm:gap-2 sm:px-4 sm:text-[15px]",
        active ? "bg-gradient-to-r from-blue-600 to-sky-500 text-white shadow-[0_10px_24px_rgba(37,99,235,0.22)]" : "bg-transparent text-zinc-800"
      )}
    >
      {icon}
...
export default function LeadForm({
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="rounded-[28px] border border-zinc-200/80 bg-white/90 p-5 shadow-[0_18px_40px_rgba(15,23,42,0.06)] backdrop-blur sm:p-6"
    >
      <div className="grid gap-5">
        ...
        <div className="grid gap-4">
          ...
          <button type="button" className="flex h-14 w-full items-center justify-center gap-3 rounded-[20px] bg-gradient-to-r from-blue-600 to-sky-500 px-5 text-[18px] font-extrabold text-white shadow-[0_14px_28px_rgba(37,99,235,0.22)]">
            <span>Рассчитать маршрут</span>
            <IconPlane />
          </button>
        </div>

        <div className="rounded-[22px] border border-zinc-200 bg-zinc-50/70 p-4 shadow-[0_8px_20px_rgba(15,23,42,0.03)]">
          <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
            <div>
              <div className="text-[18px] font-black text-zinc-900">{tripTitle}</div>
            ...
            <div className="text-left sm:text-right">
              ...
              <div className="text-[24px] font-black text-zinc-950">{finalPrice != null ? formatRub(finalPrice) : routeType === "city" ? formatFrom(CITY_BASE_PRICE[carClass]) : "—"}</div>
        ...
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button type="button" onClick={() => applyQuickTime("plus1")} className="h-10 rounded-full border border-zinc-200 bg-white px-4 text-[14px] font-bold text-zinc-700">Через 1 час</button>
            <button type="button" onClick={() => applyQuickTime("today18")} className="h-10 rounded-full border border-zinc-200 bg-white px-4 text-[14px] font-bold text-zinc-700">Сегодня 18:00</button>
            <button type="button" onClick={() => applyQuickTime("tomorrow10")} className="h-10 rounded-full border border-zinc-200 bg-white px-4 text-[14px] font-bold text-zinc-700">Завтра 10:00</button>
          </div>
        </div>
        ...
        <div>
          ...
          <textarea className="min-h-[108px] w-full rounded-[18px] border border-zinc-200 bg-white px-4 py-3 text-[16px] text-zinc-900 outline-none transition placeholder:text-zinc-400 focus:border-blue-300 focus:ring-4 focus:ring-blue-50" value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Детское кресло, багаж, рейс..." />
        </div>
        ...
        <button disabled={loading || !canSubmit} className="flex h-14 w-full items-center justify-center gap-3 rounded-[20px] bg-gradient-to-r from-blue-600 to-sky-500 px-5 text-[18px] font-extrabold text-white shadow-[0_14px_28px_rgba(37,99,235,0.22)] transition hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-60">
