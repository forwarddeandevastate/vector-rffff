...
function IconPin() {
  return <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 21s7-5.2 7-11a7 7 0 1 0-14 0c0 5.8 7 11 7 11Z" /><circle cx="12" cy="10" r="2.5" /></svg>;
}
function IconSwap() {
...
function IconCalendar() {
  return <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="5" width="18" height="16" rx="2" /><path d="M16 3v4M8 3v4M3 10h18" /></svg>;
}
function IconArrowRight() {
  return <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14" /><path d="m13 5 7 7-7 7" /></svg>;
}
...
function TypeTab({ active, label, icon, onClick }: { active: boolean; label: string; icon: ReactNode; onClick: () => void }) {
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "flex h-10 items-center justify-center gap-1 rounded-2xl px-2 text-[12px] font-extrabold transition sm:h-11 sm:gap-1.5 sm:text-[13px]",
        active ? "bg-gradient-to-r from-blue-600 to-sky-500 text-white shadow-[0_8px_18px_rgba(37,99,235,0.2)]" : "bg-transparent text-zinc-700"
      )}
    >
      {icon}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? "border-blue-200 bg-blue-50 shadow-[0_6px_14px_rgba(37,99,235,0.08)]" : "border-zinc-200 bg-white hover:bg-zinc-50"
      )}
    >
      <div className={cn("text-[13px] font-bold leading-none", active ? "text-blue-700" : "text-zinc-900")}>{title}</div>
      <div className={cn("mt-1 text-[12px] font-extrabold leading-none", active ? "text-blue-700" : "text-zinc-700")}>{price}</div>
    </button>
  );
}

function FieldLabel({ children }: { children: ReactNode }) {
  return <label className="mb-1.5 block text-[13px] font-extrabold text-zinc-900">{children}</label>;
}
...
export default function LeadForm({
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-[28px] border border-zinc-200/80 bg-white/92 p-4 shadow-[0_18px_40px_rgba(15,23,42,0.06)] backdrop-blur sm:p-5">
        ...
        <div className="grid gap-3">
          <div ref={fromBoxRef} className="relative">
            ...
            <div className="relative">
              ...
              <GooglePlacesInput
                className="h-11 w-full rounded-2xl border border-zinc-200 bg-white pl-10 pr-4 text-[15px] text-zinc-900 outline-none transition placeholder:text-zinc-400 focus:border-blue-300 focus:ring-4 focus:ring-blue-50"
                value={fromText}
            ...
            {fromOpen && fromSuggestions.length > 0 ? <div className="absolute z-30 mt-2 w-full overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-xl">{fromSuggestions.map((item) => <button key={item} type="button" className="block w-full px-4 py-2.5 text-left text-sm text-zinc-800 hover:bg-blue-50" onClick={() => { setFromText(item); setFromPlaceId(null); setFromOpen(false); }}>{item}</button>)}</div> : null}
          </div>

          <div className="relative pt-2">
            <div className="absolute left-1/2 top-0 z-20 -translate-x-1/2 -translate-y-1/2">
              <button type="button" onClick={swapPlaces} className="grid h-11 w-11 place-items-center rounded-full border border-zinc-200 bg-white text-zinc-700 shadow-[0_8px_18px_rgba(15,23,42,0.08)] transition hover:text-blue-600"><IconSwap /></button>
            </div>
            <div ref={toBoxRef} className="relative">
              ...
              {toOpen && toSuggestions.length > 0 ? <div className="absolute z-30 mt-2 w-full overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-xl">{toSuggestions.map((item) => <button key={item} type="button" className="block w-full px-4 py-2.5 text-left text-sm text-zinc-800 hover:bg-blue-50" onClick={() => { setToText(item); setToPlaceId(null); setToOpen(false); }}>{item}</button>)}</div> : null}
            </div>
          </div>

          <button type="button" className="flex h-11 w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-sky-500 px-4 text-[15px] font-extrabold text-white shadow-[0_12px_24px_rgba(37,99,235,0.2)]">
            <span>Рассчитать маршрут</span>
            <IconPlane />
          </button>
        </div>

        <div className="rounded-[22px] border border-zinc-200 bg-zinc-50/80 p-3.5 shadow-[0_8px_20px_rgba(15,23,42,0.03)]">
          <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
            <div>
              <div className="text-[17px] font-black leading-tight text-zinc-900">{tripTitle}</div>
            ...
            <div className="text-left sm:text-right">
              <div className="text-[12px] font-semibold uppercase tracking-wide text-zinc-500">цена</div>
              <div className="text-[24px] font-black leading-none text-zinc-950">{finalPrice != null ? formatRub(finalPrice) : routeType === "city" ? formatFrom(CITY_BASE_PRICE[carClass]) : "—"}</div>
            </div>
          </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="sm:col-span-2">
            ...
            <div className="relative">
              ...
              <input className="h-11 w-full rounded-2xl border border-zinc-200 bg-white pl-10 pr-4 text-[15px] text-zinc-900 outline-none transition focus:border-blue-300 focus:ring-4 focus:ring-blue-50" type="datetime-local" value={datetimeLocal} onChange={(e) => setDatetimeLocal(e.target.value)} />
            </div>
            <div className="mt-2 flex flex-wrap gap-2">
              <button type="button" onClick={() => applyQuickTime("plus1")} className="h-9 rounded-full border border-zinc-200 bg-white px-3 text-[13px] font-bold text-zinc-700">Через 1 час</button>
              <button type="button" onClick={() => applyQuickTime("today18")} className="h-9 rounded-full border border-zinc-200 bg-white px-3 text-[13px] font-bold text-zinc-700">Сегодня 18:00</button>
              <button type="button" onClick={() => applyQuickTime("tomorrow10")} className="h-9 rounded-full border border-zinc-200 bg-white px-3 text-[13px] font-bold text-zinc-700">Завтра 10:00</button>
            </div>
          </div>

          <div>
            <FieldLabel>Ваше имя</FieldLabel>
            <input className="h-11 w-full rounded-2xl border border-zinc-200 bg-white px-4 text-[15px] text-zinc-900 outline-none transition placeholder:text-zinc-400 focus:border-blue-300 focus:ring-4 focus:ring-blue-50" value={name} onChange={(e) => setName(e.target.value)} placeholder="Как к вам обращаться" autoComplete="name" />
          </div>
          <div>
            <FieldLabel>Телефон</FieldLabel>
            <input className="h-11 w-full rounded-2xl border border-zinc-200 bg-white px-4 text-[15px] text-zinc-900 outline-none transition placeholder:text-zinc-400 focus:border-blue-300 focus:ring-4 focus:ring-blue-50" value={phone} onChange={(e) => setPhone(normalizePhoneLive(e.target.value))} placeholder="+7 (___) ___-__-__" inputMode="tel" autoComplete="tel" />
          </div>
        </div>

        <div>
          <FieldLabel>Комментарий</FieldLabel>
          <textarea className="min-h-[84px] w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-[15px] text-zinc-900 outline-none transition placeholder:text-zinc-400 focus:border-blue-300 focus:ring-4 focus:ring-blue-50" value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Детское кресло, багаж, рейс..." />
        </div>

        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <label className="inline-flex cursor-pointer items-center gap-2.5 text-[15px] font-bold text-zinc-800">
          ...
          <button disabled={loading || !canSubmit} className="flex h-11 w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-sky-500 px-4 text-[15px] font-extrabold text-white shadow-[0_12px_24px_rgba(37,99,235,0.2)] transition hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto sm:min-w-[210px]">
            <span>{loading ? "Отправляем…" : "Оформить заказ"}</span>
            <IconArrowRight />
          </button>
        </div>

        {error ? <div className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-2.5 text-sm text-rose-900">{error}</div> : null}
      </div>
    </form>
  );
}
