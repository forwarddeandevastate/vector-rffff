...
export const metadata: Metadata = {
  ...
  alternates: { canonical: `${SITE_URL}/orel/kaluga` },
  robots: { index: true, follow: true },
  openGraph: {
    type: "website",
    url: `${SITE_URL}/orel/kaluga`,
    title: `Такси Орёл — ${TO_NAME} | Межгород 24/7 — ${SITE_NAME}`,
    description:
      `Трансфер Орёл — ${TO_NAME}: комфортные авто, подача по времени, стоимость согласуем заранее. Заявка онлайн 24/7.`,
    siteName: SITE_NAME,
...
export default function Page() {
  ...
  const breadcrumbsJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: `${TO_NAME}`, item: `${SITE_URL}/orel/kaluga` },
    ],
  };
  ...
  return (
    <>
      <Script
        id="ld-route-breadcrumbs-orel-kaluga"
        type="application/ld+json"
        strategy="beforeInteractive"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbsJsonLd) }}
      />
      <Script
        id="ld-route-faq-orel-kaluga"
        type="application/ld+json"
        strategy="beforeInteractive"
